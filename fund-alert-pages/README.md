...
- **零秘钥安全纯看板**：所有 Bark 推送秘钥统一收敛由 GitHub Secrets 托管巡检，前端看板完全无任何秘钥输入与外泄风险。

## 使用

1. 直接在浏览器打开 `index.html` 或通过 GitHub Pages 访问。
2. 看板自动拉取 Notion 导出的最新主仓基金，也可在本地临时添加自选观察基金。
3. 云端监控已完全托管给 GitHub Actions（每个工作日交易时段每 15 分钟自动在后台巡检并在偏离阈值时推送 Bark），无需常驻打开网页。打开本页面可随时作为实时看板查阅最新估值与走势。
